...
class ElementActions:
    ...
    def swip_left(self, count=1):
        ...
        for x in range(count):
            self.sleep(1)
            self.driver.swipe(self.width * 6 / 7, self.height / 2, self.width / 7, self.height / 2, 1000)
    ...
    def save_failure_pic(self,testName):
        try:
            # filename = '%s.%s.png' % (self.__class__.__name__, testName)
            # file_path = os.path.join('./screenshot/', filename)
            # self.driver.get_screenshot_as_file(file_path)
            allure.attach(testName,self.driver.get_screenshot_as_png(), type=AttachmentType.PNG)
        except Exception as e:
            print(e)
